        ...
        if current_index != len(question_list):
            ...
            for i in range(0, len(config.quiz[current_question]), 2):
                if i + 1 < len(config.quiz[current_question]):
                    keyboard.add(telebot.types.KeyboardButton(config.quiz[current_question][i]), telebot.types.KeyboardButton(config.quiz[current_question][i + 1]))
                else:
            ...
            keyboard.add(telebot.types.InlineKeyboardButton('Стать опекуном (Узнать подробнее)', callback_data='guardianship_system'))
